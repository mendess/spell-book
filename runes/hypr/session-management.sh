...
case "$opt" in
    lock)
        ...
        c1=$(sed -n 1p <"${CACHE}/wall_colors"  | cut -d' ' -f1 | sed 's/#//')
        c1_t=$(sed -n 1p <"${CACHE}/wall_colors"  | cut -d' ' -f2 | sed 's/#//')
        c2=$(sed -n 2p <"${CACHE}/wall_colors"  | cut -d' ' -f1 | sed 's/#//')
        c3=$(sed -n 3p <"${CACHE}/wall_colors"  | cut -d' ' -f1 | sed 's/#//')
